...
export default function UserPurchase({ onPurchaseSuccess }) {
  ...
  const handlePurchase = () => {
    ...
    setTimeout(() => {
      ...
      addNotification(NOTIF_TYPES.PURCHASE_SUCCESS, 'Slot Purchased', `You bought a ${variation} slot in "${p.campaignName}" for ₹${p.price}`, 'user', { tab: 'purchase' });
  ...
  if (purchased) {
    return (
      <div className="card" style={{ textAlign: 'center', padding: '2rem' }}>
        ...
        <button className="btn btn-primary" style={{ marginTop: '0.5rem' }} onClick={() => { setPurchased(null); setSelectedCamp(null); setSelectedAstrologerId(null); }}>Buy Another</button>
      </div>
    );
  }
